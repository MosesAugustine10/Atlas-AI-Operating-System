"""Real OpenRouter provider — makes actual HTTP calls.
...
class RealOpenRouterProvider(BaseProvider):
    ...
    def _generate_fallback(self, request: ProviderRequest) -> ProviderResponse:
        text = (
            f"[openrouter:{request.model}] {request.prompt or self._last_user(request)}"
        )
        return ProviderResponse(
            text=text,
            model=request.model,
...
__all__ = ["RealOpenRouterProvider"]
