"""Atlas Executions page — real Qt page using the ExecutionController controller.
...
if _HAS_QT:
    ...
    class ExecutionPage(QtWidgets.QWidget):  # type: ignore[misc, valid-type]
        """Real Executions page.

        Parameters:
            controller: :class:`ExecutionController` instance.
            parent: Optional Qt parent.
        """

        def __init__(self, controller: ExecutionController, parent: Any = None) -> None:
            super().__init__(parent)
            self.setObjectName("AtlasExecutionPage")
        ...
        def _build_ui(self) -> None:
            ...
            title = QtWidgets.QLabel("Executions")
        ...
        def _fetch_items(self, query: str) -> list[Any]:
            list_method = getattr(self.controller, "history", None)
        ...
        def _item_label(item: Any) -> str:
            ...
            if isinstance(item, dict):
                return str(item.get("goal", item))
            return str(getattr(item, "goal", item))

else:

    class ExecutionPage:  # type: ignore[no-redef]
