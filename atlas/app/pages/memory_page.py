"""Atlas Memory page — real Qt page using the MemoryController controller.

Browse Atlas memory entries.
"""
...
if _HAS_QT:
    ...
    class MemoryPage(QtWidgets.QWidget):  # type: ignore[misc, valid-type]
        """Real Memory page.

        Parameters:
            controller: :class:`MemoryController` instance.
            parent: Optional Qt parent.
        """

        def __init__(self, controller: MemoryController, parent: Any = None) -> None:
            super().__init__(parent)
            self.setObjectName("AtlasMemoryPage")
        ...
        def _fetch_items(self, query: str) -> list[Any]:
            list_method = getattr(self.controller, "entries", None)
        ...
        def _item_label(item: Any) -> str:
            ...
            if isinstance(item, dict):
                return str(item.get("preview", item))
            return str(getattr(item, "preview", item))

else:

    class MemoryPage:  # type: ignore[no-redef]
